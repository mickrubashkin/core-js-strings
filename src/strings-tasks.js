...
function isPalindrome(str) {
  const normalizedStr = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
  let oppositeIndex = normalizedStr.length - 1;

  for (let i = 0; i < normalizedStr.length / 2; i += 1) {
    if (normalizedStr[i] !== normalizedStr[oppositeIndex]) {
      return false;
    }
    oppositeIndex -= 1;
  }
...
function extractNameFromTemplate(value) {
  ...
  const lastName = tokens[2].substring(0, tokens[2].length - 1);
  ...
  return str
    ...
    .map((char) => {
      if (alpha.includes(char)) {
        return alpha.at(alpha.indexOf(char) - 13);
      }
      if (ALPHA.includes(char)) {
        return ALPHA.at(ALPHA.indexOf(char) - 13);
      }
